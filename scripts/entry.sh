...
function start_udev()
{
	if [ "$UDEV" == "on" ]; then
		...
		if [ "$INITSYSTEM" == "on" ]; then
			/usr/bin/systemctl mask systemd-udevd
		fi
	fi
}
...
function init_systemd()
{
	...
	sleep infinity &
	# define env DBUS_SYSTEM_BUS_ADDRESS=unix:path=/host/run/dbus/system_bus_socket
	exec env DBUS_SYSTEM_BUS_ADDRESS=unix:path=/host/run/dbus/system_bus_socket /sbin/init quiet systemd.show_status=0
}
